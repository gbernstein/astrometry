...
namespace astrometry {
  ...
  Orientation::create(const YAML::Node& node) {
    ...
    double ra = node["RA"].as<double>() * DEGREE;
    double dec = node["Dec"].as<double>() * DEGREE;
    double pa = node["PA"].as<double>() * DEGREE;
  ...
  Gnomonic::create(const YAML::Node& node) {
	...
	throw AstrometryError("Missing/bad YAML key <Orientation> for " + type());
      double ra = node["RA"].as<double>();
      double dec = node["Dec"].as<double>();
      double pa = node["PA"].as<double>();
    ...
    if (node["Xi"])
      xi = node["Xi"].as<double>() * DEGREE;
    if (node["Eta"])
      eta = node["Eta"].as<double>() * DEGREE;
